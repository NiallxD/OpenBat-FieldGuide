...
### Range Maps and Species Codes

**There's nothing to fill in here — this section is just so you know how it
works.** An entry gets its distribution map and its place in "bats near you"
automatically, keyed on the `id` slug you already gave it.

The guide used to have a `code` field for this: the 4- or 6-letter label an
OpenBat ID model puts on a detection (`MYCA`, `PIPPIP`), which a contributor
was asked to mint for any species no model names. It's gone. It was an
unanswerable question — a classifier code is a fact about our models, not
about the bat, and you had no way to look one up — and any entry that left it
blank quietly got no map at all.

Codes still exist inside the app, and they matter there: a model emits a code,
and every detection is stored, written into the WAV's GUANO tag and exported
under it. But a species no model names can never be a detection, so it never
needs one. If a future model adds a species the guide already describes, the
app picks the code up from the model itself — no guide edit required.
...
for occurrence records for every species an ID model names (keyed by the
model's code) plus every guide entry no model names (keyed by its `id` slug —
